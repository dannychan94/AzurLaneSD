var load = function(
    ...
    var setUrlParam = function() {
        ...
        url +=
            "?" +
            $("#skeletonList option:selected").text() +
            "&" +
    ...
    var setupUI = function() {
        ...
        $(function() {
            ...
            $(document).on("click", function(e) {
                ...
                while (elem) {
                    if (
                        elem.id &&
                        (elem.id == "skeletonList" || elem.id == "skeletonBox")
                    ) {
                        return;
                    }
            ...
            for (var skeletonName in nameList) {
                if (skeletonName.indexOf(this.value) != -1) {
                    var option = $("<option></option>")
                        .attr("value", skeletonName)
                        .text(skeletonName);
                    if (skeletonName === currentSkeleton)
        ...
        $("#share").on("mouseenter", function() {
            showMessage(
                "Haz clic en el botón de compartir en la parte superior izquierda para compartir tu personaje y acciones actuales con los demás",
                4000
            );
        });
        $("#share").on("click", function() {
    ...
    var init = function() {
        ...
        showMessage(
            "Arrastra el ratón para moverte, haz clic en la esquina inferior izquierda para restablecer",
            4000
        );
    ...
    var loadSkeleton = function(
        ...
        if (byJson) {
            ...
            skeletonData = skeletonJson.readSkeletonData(
                assetManager.get(path + ".json")
            );
        } else {
            skeletonData = skeletonBinary.readSkeletonData(
                assetManager.get(path + ".skel")
            );
        }
    ...
    var resize = function() {
        ...
        mvp.ortho2d(
            centerX - offsetX * scaling - width / 2,
            centerY + offsetY * scaling - height / 2,
            width,
            height
        );
